...
import { BufferGeometry, Color, DoubleSide, Float32BufferAttribute, type Group, LineBasicMaterial, Shape, Vector3 } from 'three'
...
const createWallGradientMaterial = (zoneColor: string) => {
  ...
  // Fade opacity from full at bottom to 0 at top, controlled by uOpacity uniform
  const finalOpacity = float(1).sub(gradientT).mul(opacity)
...
/**
 * Creates outline geometry for zone boundaries
 * Draws a line loop around the polygon perimeter at floor level
 */
const createOutlineGeometry = (polygon: Array<[number, number]>): BufferGeometry => {
  ...
  for (let i = 0; i < polygon.length; i++) {
    const [x, z] = polygon[i]!
    positions.push(x, Y_OFFSET + 0.05, z)
  }

  // Close the loop by adding first vertex again
  const [x0, z0] = polygon[0]!
  positions.push(x0, Y_OFFSET + 0.05, z0)
...
export const ZoneRenderer = ({ node }: { node: ZoneNode }) => {
  ...
  // Create outline geometry from polygon
  const outlineGeometry = useMemo(() => {
    if (!node?.polygon || node.polygon.length < 2) return null
    return createOutlineGeometry(node.polygon)
  }, [node?.polygon])
  ...
  if (!(node && floorShape && wallGeometry && floorMaterial && wallMaterial && outlineGeometry)) {
    return null
  }
  ...
  return (
    <group ref={ref} {...handlers} userData={{ labelPosition: [centroid[0], 1, centroid[1]] }}>
      ...
      <lineSegments
        ...
        layers={ZONE_LAYER}
        name="outline"
        material={new LineBasicMaterial({ color: node.color, linewidth: 2 })}
      />
    </group>
  )
}
